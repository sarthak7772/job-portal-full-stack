...
const RecuiterLogin = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        if (state === "Sign Up" && !isTextDataSubmited) {
            ...
            // Proceed with final account creation (e.g., send data)
    ...
    return (
        <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
            <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
                ...
                {
                    state === 'Login'
                        ? <p className='mt-5 text-center cursor-pointer'>Don't have an account? <span className='text-blue-600' onClick={() => setState("Sign Up")}>Sign Up</span></p>
                        : <p className='mt-5 text-center cursor-pointer'>Already have an acoount? <span className='text-blue-600' onClick={() => setState("Login")}>Login</span></p>
                }
